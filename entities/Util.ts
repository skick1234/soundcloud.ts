...
export class Util {
    ...
    if (trackResolvable.hasOwnProperty("downloadable")) {
      ...
      if (track.downloadable === true) {
        ...
        dest = path.extname(dest)
          ? dest
          : path.join(folder, `${track.title.replace(/\//g, "")}.${result.headers["x-amz-meta-file-type"]}`)
  ...
  public downloadTracks = async (
    tracks: SoundcloudTrack[] | SoundcloudTrackV2[] | string[],
    dest?: string,
    limit?: number
  ) => {
  ...
  public streamTrack = async (
    trackResolvable: string | SoundcloudTrack | SoundcloudTrackV2
