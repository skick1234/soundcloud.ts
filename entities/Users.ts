...
export class Users {
    ...
    public searchAlt = async (query: string) => {
        const html = await request(`https://soundcloud.com/search/people?q=${query}`, {headers: this.api.headers}).then((r) => r.body.text())
    ...
    public getAlt = async (url: string) => {
        ...
        const songHTML = await request(url, {headers: this.api.headers}).then((r) => r.body.text())
        const json = JSON.parse(songHTML.match(/(\[{)(.*)(?=;)/gm)[0])
        const user = json[json.length - 1].data
